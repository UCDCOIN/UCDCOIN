...
###   unitedcontinentaldollarcore.org and unitedcontinentaldollar.org and compares them.
...
WORKINGDIR="/tmp/unitedcontinentaldollar_verify_binaries"
TMPFILE="hashes.tmp"

SIGNATUREFILENAME="SHA256SUMS.asc"
RCSUBDIR="test"
HOST1="https://unitedcontinentaldollarcore.org"
HOST2="https://unitedcontinentaldollar.org"
BASEDIR="/bin/"
VERSIONPREFIX="unitedcontinentaldollar-core-"
RCVERSIONSTRING="rc"
...
if [ -n "$1" ]; then
   #let's also check if the version number includes the prefix 'unitedcontinentaldollar-',
...
if ! WGETOUT=$(wget -N -O "$SIGNATUREFILENAME.2" "$HOST2$BASEDIR$SIGNATUREFILENAME" 2>&1); then
   echo "unitedcontinentaldollar.org failed to provide signature file, but unitedcontinentaldollarcore.org did?"
   echo "wget output:"
   # shellcheck disable=SC2001
   echo "$WGETOUT"|sed 's/^/\t/g'
...
if [ "$SIGFILEDIFFS" != "" ]; then
   echo "unitedcontinentaldollar.org and unitedcontinentaldollarcore.org signature files were not equal?"
...
if [ $RET -ne 0 ]; then
   if [ $RET -eq 1 ]; then
      ...
      echo "gpg error. Do you have the UnitedContinentalDollar Core binary release signing key installed?"
   fi
