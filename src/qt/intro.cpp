    ...
    m_prune_target_gb{GetPruneTargetGB()}
{
    ...
    ui->lblExplanation1->setText(ui->lblExplanation1->text()
        ...
        .arg(2021)
        .arg(tr("UnitedContinentalDollar"))
    );
...
bool Intro::showIfNeeded(bool& did_show_intro, bool& prune)
{
    ...
    if(!fs::exists(GUIUtil::qstringToBoostPath(dataDir)) || gArgs.GetBoolArg("-choosedatadir", DEFAULT_CHOOSE_DATADIR) || settings.value("fReset", false).toBool() || gArgs.GetBoolArg("-resetguisettings", false))
    {
        ...
        intro.setWindowIcon(QIcon(":icons/unitedcontinentaldollar"));
        did_show_intro = true;
     ...
     * override -datadir in the unitedcontinentaldollar.conf file in the default data directory
     * (to be consistent with unitedcontinentaldollard behavior)
     */
    if(dataDir != GUIUtil::getDefaultDataDirectory()) {
...
void Intro::UpdatePruneLabels(bool prune_checked)
{
    ...
    ui->sizeWarningLabel->setText(
        tr("%1 will download and store a copy of the UnitedContinentalDollar block chain.").arg(PACKAGE_NAME) + " " +
