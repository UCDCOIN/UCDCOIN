...
#include <config/unitedcontinentaldollar-config.h> /* for USE_QRCODE */
#endif
...
void ReceiveRequestDialog::setInfo(const SendCoinsRecipient &_info)
{
    ...
    QString uri = GUIUtil::formatUnitedContinentalDollarURI(info);
...
{
    if (!model) return;
    ui->amount_content->setText(UnitedContinentalDollarUnits::formatWithUnit(model->getOptionsModel()->getDisplayUnit(), info.amount));
}

void ReceiveRequestDialog::on_btnCopyURI_clicked()
{
    GUIUtil::setClipboard(GUIUtil::formatUnitedContinentalDollarURI(info));
}
