...
void PSBTOperationsDialog::saveTransaction() {
    ...
    for (const CTxOut& out : m_transaction_data.tx->vout) {
        ...
        QString amount = UnitedContinentalDollarUnits::format(m_wallet_model->getOptionsModel()->getDisplayUnit(), out.nValue);
...
std::string PSBTOperationsDialog::renderTransaction(const PartiallySignedTransaction &psbtx)
{
    ...
    for (const CTxOut& out : psbtx.tx->vout) {
        ...
        tx_description.append(tr(" * Sends %1 to %2")
            .arg(UnitedContinentalDollarUnits::formatWithUnit(UnitedContinentalDollarUnits::UCD, out.nValue))
    ...
    if (!*analysis.fee) {
        ...
        tx_description.append(UnitedContinentalDollarUnits::formatWithUnit(UnitedContinentalDollarUnits::UCD, *analysis.fee));
        ...
        tx_description.append(QString("<b>%1</b>: <b>%2</b>").arg(tr("Total Amount"))
            .arg(UnitedContinentalDollarUnits::formatHtmlWithUnit(m_client_model->getOptionsModel()->getDisplayUnit(), totalAmount)));
