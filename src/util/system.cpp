...
const char * const UNITEDCONTINENTALDOLLAR_CONF_FILENAME = "unitedcontinentaldollar.conf";
const char * const UNITEDCONTINENTALDOLLAR_SETTINGS_FILENAME = "settings.json";
 ...
 * https://github.com/unitedcontinentaldollar/unitedcontinentaldollar/pull/16097#issuecomment-514627823
 */
...
bool ArgsManager::ParseParameters(int argc, const char* const argv[], std::string& error)
{
    ...
    for (int i = 1; i < argc; i++) {
        ...
        if (key == "-") break; //unitedcontinentaldollar-tx using stdin
        std::string val;
...
bool ArgsManager::GetSettingsPath(fs::path* filepath, bool temp) const
{
    ...
    if (filepath) {
        std::string settings = GetArg("-settings", UNITEDCONTINENTALDOLLAR_SETTINGS_FILENAME);
...
#else
    const char* pszModule = "unitedcontinentaldollar";
#endif
    if (pex)
        return strprintf(
...
{
    // Windows: C:\Users\Username\AppData\Roaming\UnitedContinentalDollar
    // macOS: ~/Library/Application Support/UnitedContinentalDollar
    // Unix-like: ~/.unitedcontinentaldollar
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "UnitedContinentalDollar";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // macOS
    return pathRet / "Library/Application Support/UnitedContinentalDollar";
#else
    // Unix-like
    return pathRet / ".unitedcontinentaldollar";
#endif
#endif
}
...
bool ArgsManager::ReadConfigFiles(std::string& error, bool ignore_invalid_keys)
{
    ...
    const std::string confPath = GetArg("-conf", UNITEDCONTINENTALDOLLAR_CONF_FILENAME);
