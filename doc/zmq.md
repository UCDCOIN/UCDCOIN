...
The UnitedContinentalDollar Core daemon can be configured to act as a trusted "border
router", implementing the unitedcontinentaldollar wire protocol and relay, making
...
The ZeroMQ feature in UnitedContinentalDollar Core requires the ZeroMQ API >= 4.0.0
...
during the *configure* step of building unitedcontinentaldollard:
...
For instance:
    ...
    $ unitedcontinentaldollard -zmqpubhashtx=tcp://127.0.0.1:245797 \
               -zmqpubhashtx=tcp://192.168.1.2:245797 \
               -zmqpubrawtx=ipc:///tmp/unitedcontinentaldollard.tx.raw \
...
These options can also be provided in unitedcontinentaldollar.conf.
...
From the perspective of unitedcontinentaldollard, the ZeroMQ socket is write-only; PUB
sockets don't even have a read function. Thus, there is no state
introduced into unitedcontinentaldollard directly. Furthermore, no information is
...
using. UnitedContinentalDollard appends an up-counting sequence number to each
